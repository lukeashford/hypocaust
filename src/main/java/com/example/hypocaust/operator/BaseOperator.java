 ...
 * Provides consistent error handling and event emission.
 */
@Slf4j
public abstract class BaseOperator implements Operator {
  ...
  public final OperatorResult execute(Map<String, Object> rawInputs, String todoPath) {
    ...
    try {
      ...
      eventService.publish(new OperatorStartedEvent(projectId, operatorName, normalizedInputs, todoPath));
      ...
      if (!validationResult.ok()) {
        ...
        eventService.publish(
            new OperatorFailedEvent(projectId, operatorName, normalizedInputs, failure.message(), todoPath));
        return failure;
      }
      ...
      if (result.ok()) {
        eventService.publish(
            new OperatorFinishedEvent(projectId, operatorName, normalizedInputs, result.outputs(), todoPath));
      } else {
        eventService.publish(
            new OperatorFailedEvent(projectId, operatorName, normalizedInputs, result.message(), todoPath));
      }

      return result;
    } catch (ArtifactNotFoundException e) {
      return handleException(projectId, operatorName, rawInputs, todoPath,
  ...
  private OperatorResult handleException(UUID projectId, String operatorName,
      Map<String, Object> inputs, String todoPath, String message) {
    eventService.publish(new OperatorFailedEvent(projectId, operatorName, inputs, message, todoPath));
