...
public class ArtifactUploadService {
  ...
  private static ArtifactKind kindFromMimeType(String mimeType) {
    if (mimeType == null) return ArtifactKind.OTHER;
    if (mimeType.startsWith("image/")) return ArtifactKind.IMAGE;
    if (mimeType.startsWith("audio/")) return ArtifactKind.AUDIO;
    if (mimeType.startsWith("video/")) return ArtifactKind.VIDEO;
    if (mimeType.startsWith("text/")) return ArtifactKind.TEXT;
    if (mimeType.equals("application/pdf")) return ArtifactKind.PDF;
    return ArtifactKind.OTHER;
  }

  private static String sanitizeName(String filename) {
    if (filename == null || filename.isBlank()) return "upload-" + UUID.randomUUID();
