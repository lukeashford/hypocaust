...
class AnalyzeRequest(BaseModel):
    input_url: str = Field(description="URL or local path of the media file to analyze")


class AnalyzeResponse(BaseModel):
    status: str = "completed"
    format: Optional[dict] = Field(None, description="Container format details")
    streams: Optional[list] = Field(None, description="Audio/video stream details")
    loudness: Optional[dict] = Field(None, description="EBU R128 loudness measurements (if audio present)")


class ConvertRequest(BaseModel):
    input_url: str = Field(description="URL or local path of the input media file")
    ffmpeg_args: list[str] = Field(description="FFmpeg CLI arguments for the conversion/filter pipeline")
    output_format: str = Field(default="mp4", description="Output file extension (e.g. mp4, mp3, png, wav)")


class ConvertResponse(BaseModel):
    status: str = "completed"
    output_url: str = Field(description="Path to the output file in the data volume")
    ffmpeg_stderr: Optional[str] = Field(None, description="FFmpeg stderr (for diagnostics)")
...
def _download_input(url: str, workdir: Path) -> Path:
    ...
    if url.startswith(("http://", "https://")):
        ...
        if result.returncode != 0:
            ...
            if result.returncode != 0:
                raise HTTPException(status_code=400, detail=f"Failed to download input: {result.stderr}")
        return dest
    return Path(url)
...
@app.post("/api/v1/analyze", response_model=AnalyzeResponse, dependencies=[Security(verify_key)])
def analyze(req: AnalyzeRequest):
    ...
    with tempfile.TemporaryDirectory(dir=DATA_DIR) as workdir:
        ...
        if result.returncode != 0:
            raise HTTPException(status_code=400, detail=f"ffprobe failed: {result.stderr}")
...
@app.post("/api/v1/convert", response_model=ConvertResponse, dependencies=[Security(verify_key)])
def convert(req: ConvertRequest):
    ...
    with tempfile.TemporaryDirectory(dir=DATA_DIR) as workdir:
        ...
        cmd = ["ffmpeg", "-y", "-i", str(input_path)] + req.ffmpeg_args + [str(output_file)]
        result = _run(cmd)

        if result.returncode != 0:
            raise HTTPException(status_code=400, detail=f"ffmpeg failed: {result.stderr[-2000:]}")
