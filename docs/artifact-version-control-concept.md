...
This document describes the simplified artifact version control system for Hypocaust. It replaces the overly complex branch-based system with a streamlined commit graph approach.

---

## Core Philosophy

1. **Stateless backend**: No concept of "checkout". The backend knows the full commit graph; the frontend tells it which commit to start from.
2. **Name-based artifacts**: Artifacts have semantic file names (e.g., `protagonists_dog`, `script`) instead of UUIDs for LLM-friendly references.
3. **Commit-time persistence**: Operators don't save artifacts during execution. They record intentions in a diff; the commit process downloads and stores files.
4. **Everything is a task**: All operations (create, edit, delete) flow through the DecomposingOperator. No direct REST manipulation of artifacts.
5. **Lean implementation**: Projects have ~10 artifacts max. No need for semantic embeddings or complex graph traversal.
...
- Removed: `anchorDescription`, `anchorRole`, `anchorTags`, `supersededById`, `branchId`, `derivedFrom`
...
class ArtifactVersionManagementService {
     ...
     * If run has no commit yet: return artifacts at baseCommit + pending changes.
     */
...
class RunContext {
    ...
    /**
     * Add a new artifact.
     * @throws ArtifactExistsException if name already exists in current state
     */
    void addArtifact(PendingArtifact artifact);

    /**
     * Edit an existing artifact (creates new version).
     * @throws ArtifactNotFoundException if name doesn't exist in current state
     */
    void editArtifact(String name, PendingArtifact newVersion);

    /**
     * Delete an artifact (soft delete).
     * Does not throw if artifact doesn't exist.
     */
    ...
    /**
     * Get current artifact state (base commit + pending).
     */
...
Accumulator for run changes, used by RunContext.
...
A **tool** (not an operator) that provides project context to operators. Answers questions in natural language.
...
class ProjectContextTool implements Tool {
     ...
     * - "What is the artifact name for the picture of our protagonist?"
    ...
    // === Deterministic methods (no LLM needed) ===

    List<ArtifactSnapshot> getCurrentArtifacts();
    List<Commit> getCommitHistory();
    List<Artifact> getVersionHistory(String artifactName);
    Optional<Artifact> getArtifactByName(String name);
}
```

**Usage**: Operators call `projectContextTool.ask("What is the artifact name for the dog picture?")` to resolve natural language references to artifact names.
...
**Key change**: Operator doesn't call `ArtifactService.schedule()` or `updateArtifact()`. It just records the pending artifact. The commit process downloads and stores it.
...
**Critical**: The commit process (create commit + update run status) must be transactional. A run with a commitId MUST be COMPLETED.
...
ALTER TABLE commit ADD COLUMN IF NOT EXISTS project_id UUID NOT NULL REFERENCES project(id);
ALTER TABLE commit ADD COLUMN IF NOT EXISTS message TEXT;

-- Modify artifact table
ALTER TABLE artifact ADD COLUMN IF NOT EXISTS name VARCHAR(100);
ALTER TABLE artifact ADD COLUMN IF NOT EXISTS description TEXT;
ALTER TABLE artifact ADD COLUMN IF NOT EXISTS prompt TEXT;
ALTER TABLE artifact ADD COLUMN IF NOT EXISTS model VARCHAR(100);
ALTER TABLE artifact ADD COLUMN IF NOT EXISTS deleted BOOLEAN DEFAULT FALSE;
...
ALTER TABLE event ADD CONSTRAINT event_type_check CHECK (type IN (
    'artifact.scheduled', 'artifact.created', 'artifact.cancelled', 'artifact.deleted',
    'run.created', 'run.scheduled', 'run.started', 'run.completed',
    'task.progress.updated',
    'tool.calling',
    'operator.started', 'operator.finished', 'operator.failed',
    'error'
));
```
...
Recommendation: **Option A** - The decomposer knows the project context and can assign meaningful names. Operators just use them.
...
The version is incremented automatically by `RunContext.editArtifact()` based on the highest existing version for that name.
